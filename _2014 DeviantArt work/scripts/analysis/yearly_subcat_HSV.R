...
res = as.data.frame(dt.reduced[,lapply(.SD, FUN = function(x) as.double(mean(x)/sd(x))), by = list(Year, cat.2), .SDcols = 45:68])
res = subset(res, Year != 2000 & Year != 2001 & Year != 2002)
rm(dt)
rm(dt.reduced)
...
p = ggplot(pca.H, aes(x = Comp.1, y = Comp.2, label = Year, colour = cat.2)) + geom_text() + ggtitle("PCA of Hue Distributions")
#ggsave(file = "./figures/Hue_PCA.pdf", p)
pca.S = cbind(res.S[,c(1,2)], return_PCA(res.S[,-c(1,2)]))
p = ggplot(pca.S, aes(x = Comp.1, y = Comp.2, label = Year, colour = cat.2)) + geom_text() + ggtitle("PCA of Saturation Distributions")
#ggsave(file = "./figures/Saturation_PCA.pdf", p)
pca.V = cbind(res.V[,c(1,2)], return_PCA(res.V[,-c(1,2)]))
p = ggplot(pca.V, aes(x = Comp.1, y = Comp.2, label = Year, colour = cat.2)) + geom_text() + ggtitle("PCA of Value Distributions")
...
#####-------------------------------------
first.row = c(0:(length(unique(res.S$Year))-1))
...
hours.HSV.list = list()
categories = unique(res.H$cat.2)
library(plyr)
for (i in c(1:length(categories))){
  res.H.temp = subset(res.H, cat.2 == categories[i])
  res.H.temp$cat.2 = NULL
  res.S.temp = subset(res.S, cat.2 == categories[i])
  res.S.temp$cat.2 = NULL
  res.V.temp = subset(res.V, cat.2 == categories[i])
  res.V.temp$cat.2 = NULL
...
p = ggplot(hours.HSV.res, aes(x = years.dist, y = hue.dist)) + facet_wrap(~ cat.2) + geom_point(size = 3, alpha = .9) + stat_smooth(method = lm) + xlab("Difference in years") + ylab("Difference in Hue Distributions")
#ggsave(file = "./figures/Hue_time.pdf", p)
p = ggplot(hours.HSV.res, aes(x = years.dist, y = saturation.dist)) + facet_wrap(~ cat.2) + geom_point(size = 3, alpha = .9) + stat_smooth(method = lm) + xlab("Difference in years") + ylab("Difference in Saturation Distributions")
#ggsave(file = "./figures/Saturation_time.pdf", p)
p = ggplot(hours.HSV.res, aes(x = years.dist, y = value.dist))+ facet_wrap(~ cat.2)  + geom_point(size = 3, alpha = .9) + stat_smooth(method = lm) + xlab("Difference in years") + ylab("Difference in Value Distributions")
